...
int main(int argc, char *argv[]) {
	int numSigs, scnt;
	pid_t childPid;
	sigset_t blockedMask, fullMask;

	if (argc != 2 || strcmp(argv[1], "--help") == 0) usageErr("%s num-sigs\n", argv[0]);
	...
	if (sigprocmask(SIG_SETMASK, &blockedMask, NULL) == -1) errExit("sigprocmask");
	...
	case 0: /* child */
		for (scnt = 0; scnt < numSigs; scnt++) {
			if (kill(getppid(), TESTSIG) == -1) errExit("kill");

			int r = sigwaitinfo(&blockedMask, NULL);
			if (r < 0) {
				errExit("sigwaitinfo");
			} else if (r == TESTSIG) {
				cnt++;
	...
	default: /* parent */
		for (scnt = 0; scnt < numSigs; scnt++) {
			...
			if (kill(childPid, TESTSIG) == -1) errExit("kill");
		}
